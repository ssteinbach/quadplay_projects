...
const $frame = (function quadplay_main_loop() {
    try {
        if (($gameMode.$name[0] !== '$') && ($goToSystemMenu || gamepad_array[0].$pp || gamepad_array[1].$pp || gamepad_array[2].$pp || gamepad_array[3].$pp)) { push_mode($SystemMenu, $goToSystemMenu); } $goToSystemMenu = false;
        $processFrameHooks();
        ${sectionTable.frame.jsCode}
        $show();
    } catch (ex) {
        if (! ex.nextMode) { throw ex; } else { $resetTouchInput(); $updateInput(); }
    }
});
